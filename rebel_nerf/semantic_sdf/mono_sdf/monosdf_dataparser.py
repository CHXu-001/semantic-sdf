...
def get_image(
    image_filename, alpha_color=None
    ...
    assert np_image.shape[2] in [
        3,
        4,
    ], f"Image shape of {np_image.shape} is in correct."
    image = torch.from_numpy(np_image.astype("float32") / 255.0)
    if alpha_color is not None and image.shape[-1] == 4:
        assert image.shape[-1] == 4
        image = image[:, :, :3] * image[:, :, -1:] + alpha_color * (
            1.0 - image[:, :, -1:]
        )
    else:
        image = image[:, :, :3]
    ...
    def _generate_dataparser_outputs(
        ...
        if pairs_path.exists() and split == "train" and self.config.load_pairs:
            ...
            for sources_line in pairs:
                sources_array = [
                    int(split_ext(img_name)) for img_name in sources_line.split(" ")
                ]

...
class Parameters:
    ...
    output_size: list[int] = field(default_factory=lambda: [800, 600])
...
def main() -> None:
    ...
    for thermal_img in thermal_data_folder.iterdir():
        ...
        mlflow.log_image(pseudo_tex_image, file_name.split(".")[0] + "_pseudo_tex.png")

        cv2.imwrite(
            str(
                Path(
                    str(saving_folder),
                    str(file_name.split(".")[0]) + "_pseudo_tex.png",
                )
            ),
            pseudo_tex_image,
        )
